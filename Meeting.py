...
class Meeting(User, AutoRepairShop): # Тут сlass Meeting наследует родителей User и AutoRepairShop что бы иметь доступ к его аргументам и методам
    def __init__(self, name, usercarmodel, users, carname, cars, meeting=list()):
        self.name = name
        self.usercarmodel = usercarmodel
        self.users = users
        self.company_name = carname
        self.cars = cars
        self.meeting = meeting


    def AddMeetingInArray(self, name, usercarmodel, carname, time): #Метод для добавления встречи принимает 4 параметра (Вводится в ручную)
        mas = list()
    ...
    def PrintMeeting(self): #Метод для вывода встреч в красивом виде
        for i in self.meeting.__iter__():
            print('\n Name: {}\n Car model: {}\n Company name: {}\n Time: {}\n'.format(i[0], i[1], i[2], i[3]))

    def removeMeeting(self, name): #Метод удаление встречи,принимает один параметр
        for i in self.meeting.__iter__():
            if name == i[0]:
                self.meeting.remove(i)
                print("A meeting {}".format(name) + ' deleted')

    def ChangeTimeMeeting(self, namemet, newtime):  #Метод изменение времени,получает два параметра (Вводится в ручную)
        for i in self.meeting.__iter__():
            if namemet == i[0]:
                i[2] = newtime
